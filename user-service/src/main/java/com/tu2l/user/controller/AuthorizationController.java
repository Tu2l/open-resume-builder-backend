...
public class AuthorizationController extends BaseAuthorizationController implements AuthorizationApi {
    ...
    public ResponseEntity<AuthorizationResponse> checkPermission(String resource, String action, String userRole) {
        ...
        boolean allowed = authorizationService.hasPermission(parseRoleOrGuest(userRole), resource, action);
    ...
    public ResponseEntity<AuthorizationResponse> getMyPermissions(String userRole) {
        return ok(AuthorizationResponse.builder()
                        .permissions(toNames(authorizationService.getPermissionsForRole(parseRoleOrGuest(userRole)))).build(),
                "Permissions retrieved successfully");
    }

    private UserRole parseRoleOrGuest(String userRole) {
        try {
